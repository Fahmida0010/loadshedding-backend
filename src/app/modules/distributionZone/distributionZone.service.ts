...
const createDistributionZone = async (
  payload: ICreateDistributionZone,
) => {
  ...
  const existingZone =
    await prisma.distributionZone.findFirst({
      where: {
        deletedAt: null,
        OR: [
          {
  ...
  if (existingZone) {
    if (
      existingZone.code.toLowerCase() ===
      normalizedCode.toLowerCase()
    ) {
      throw new AppError(
        409,
        "Distribution zone code already exists",
      );
    }

    throw new AppError(
      409,
      "Distribution zone name already exists",
    );
  }

  const result =
    await prisma.distributionZone.create({
      data: {
        name: normalizedName,
        code: normalizedCode,
        description:
 ...
 */
const getAllDistributionZones = async (
  query: IDistributionZoneQuery,
) => {
 ...
 */
const getDistributionZoneById = async (
  id: string,
) => {
  const result =
    await prisma.distributionZone.findFirst({
 ...
 */
const deleteDistributionZone = async (
  id: string,
) => {
  const existingZone =
    await prisma.distributionZone.findFirst({
  ...
  const activeSubstationCount =
    await prisma.substation.count({
      where: {
        zoneId: id,
        deletedAt: null,
      },
