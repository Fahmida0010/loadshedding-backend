...
import type { IAreaQuery, ICreateArea, IUpdateArea } from "./area.interface";
...
const createArea = async (payload: ICreateArea) => {
	...
	if (existingCode) {
		throw new AppError(409, `Area code "${normalizedCode}" already exists`);
	}
...
const updateArea = async (id: string, payload: IUpdateArea) => {
	 ...
	 */
	const targetName =
		payload.name !== undefined ? payload.name.trim() : existingArea.name;
