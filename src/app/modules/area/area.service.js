...
const createArea = async (payload) => {
     ...
     */
    const existingCode = await prisma_1.prisma.area.findFirst({
        where: {
            code: normalizedCode,
        },
    });
    if (existingCode) {
        throw new AppError_1.AppError(409, `Area code "${normalizedCode}" already exists`);
    }
    /*
     * Area name must be unique inside the same feeder.
     */
    const existingName = await prisma_1.prisma.area.findFirst({
        where: {
    ...
    if (existingName) {
        throw new AppError_1.AppError(409, `Area name "${normalizedName}" already exists in this feeder`);
    }
    const result = await prisma_1.prisma.area.create({
        data: {
            feederId: payload.feederId,
            name: normalizedName,
            code: normalizedCode,
...
const getAreaById = async (id) => {
    const result = await prisma_1.prisma.area.findFirst({
        where: {
            id,
            deletedAt: null,
        },
...
const updateArea = async (id, payload) => {
    ...
    const targetFeederId = payload.feederId ?? existingArea.feederId;
    /*
     ...
     */
    const targetName = payload.name !== undefined ? payload.name.trim() : existingArea.name;
    const duplicateName = await prisma_1.prisma.area.findFirst({
        where: {
    ...
    if (duplicateName) {
        throw new AppError_1.AppError(409, `Area name "${targetName}" already exists in this feeder`);
    }
    ...
    const result = await prisma_1.prisma.area.update({
        where: {
            id,
        },
        data: {
...
const deleteArea = async (id) => {
    ...
    if (area._count.users > 0) {
        throw new AppError_1.AppError(409, "Area cannot be deleted because customers are connected to it");
    }
    if (area._count.schedules > 0) {
        throw new AppError_1.AppError(409, "Area cannot be deleted because it contains load-shedding schedules");
    }
    if (area._count.unexpectedOutages > 0) {
        throw new AppError_1.AppError(409, "Area cannot be deleted because it contains outage reports");
    }
    const result = await prisma_1.prisma.area.update({
        where: {
            id,
