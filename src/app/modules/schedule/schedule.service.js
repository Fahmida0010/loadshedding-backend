...
const checkScheduleConflict = async (areaId, scheduledStart, scheduledEnd, excludeScheduleId) => {
    const conflictingSchedule = await prisma_1.prisma.loadSheddingSchedule.findFirst({
        where: {
            areaId,
            deletedAt: null,
    ...
    if (conflictingSchedule) {
        throw new AppError_1.AppError(409, `Schedule conflicts with "${conflictingSchedule.title}" from ${conflictingSchedule.scheduledStart.toISOString()} to ${conflictingSchedule.scheduledEnd.toISOString()}`);
    }
};
const createSchedule = async (userId, payload) => {
    ...
    if (Number.isNaN(scheduledStart.getTime()) ||
        ...
        throw new AppError_1.AppError(400, "Invalid schedule date");
    }
    if (scheduledEnd <= scheduledStart) {
        throw new AppError_1.AppError(400, "Scheduled end time must be later than start time");
    }
    ...
    if (query.startDate || query.endDate) {
        ...
        if (query.startDate) {
            ...
            if (Number.isNaN(startDate.getTime())) {
                throw new AppError_1.AppError(400, "Invalid startDate");
            }
        ...
        if (query.endDate) {
            ...
            if (Number.isNaN(endDate.getTime())) {
                throw new AppError_1.AppError(400, "Invalid endDate");
            }
...
const getScheduleById = async (id) => {
    ...
    if (!schedule) {
        throw new AppError_1.AppError(404, "Schedule not found");
    }
    return schedule;
};
const updateSchedule = async (id, payload) => {
    ...
    const recurrenceRule = payload.recurrenceRule !== undefined
        ? payload.recurrenceRule
        : existingSchedule.recurrenceRule;
    if (isRecurring && !recurrenceRule) {
        throw new AppError_1.AppError(400, "Recurrence rule is required for a recurring schedule");
    }
