...
const allowedStatusTransitions: Record<TOutageStatus, TOutageStatus[]> = {
...
const validateEstimatedRestoreTime = (
	...
	if (Number.isNaN(restoreDate.getTime())) {
		throw new AppError(400, "Invalid estimated restoration date");
	}

	return restoreDate;
};
const createOutage = async (reportedById: string, payload: ICreateOutage) => {
	await checkAreaExists(payload.areaId);

	// একই customer-এর একই active report আগে থেকেই আছে কি না
	const existingOutage = await prisma.unexpectedOutage.findFirst({
		where: {
	...
	const estimatedRestoreAt = validateEstimatedRestoreTime(
...
const getAllOutages = async (query: IOutageQuery) => {
	...
	if (query.reportedFrom || query.reportedTo) {
		...
		if (query.reportedFrom) {
			...
			if (Number.isNaN(reportedFrom.getTime())) {
				throw new AppError(400, "Invalid reportedFrom date");
			}
		...
		if (query.reportedTo) {
			...
			if (Number.isNaN(reportedTo.getTime())) {
				throw new AppError(400, "Invalid reportedTo date");
			}
...
const getMyOutages = async (reportedById: string, query: IOutageQuery) => {
	const page = Math.max(Number(query.page) || 1, 1);
...
const getOutageById = async (id: string) => {
	const outage = await prisma.unexpectedOutage.findFirst({
		where: {
			id,
			deletedAt: null,
		},
...
const updateOutage = async (id: string, payload: IUpdateOutage) => {
	...
	let estimatedRestoreAt: Date | null | undefined;

	if (payload.estimatedRestoreAt !== undefined) {
		estimatedRestoreAt = validateEstimatedRestoreTime(
			payload.estimatedRestoreAt,
		);
	}
...
const updateOutageStatus = async (id: string, payload: IUpdateOutageStatus) => {
	const existingOutage = await prisma.unexpectedOutage.findFirst({
		where: {
			id,
			deletedAt: null,
	...
	if (existingOutage.status === payload.status) {
		throw new AppError(400, `Outage status is already ${payload.status}`);
	}
	...
	let restoredAt: Date | null | undefined;
...
const deleteOutage = async (id: string) => {
	...
	if (
		existingOutage.status === "IN_PROGRESS" ||
